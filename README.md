...
Show the full screen viewer with passed image on the show method. You can pass two different resolution of the image as first and second argument (optional). See why do you need it at [better image loading](#better-image-loading) section. The third parameter is for the viewbox size of the root svg. The fourth parameter is for an array of svg objects which contain the image sources and viewbox sizes. An svg object can also take a children attribute which is an array of all the paths of the svg and shows a new image when clicking on a path.
